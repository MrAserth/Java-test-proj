...
public class ProjectileManager {
	...
	private ArrayList<Explosion> explosions = new ArrayList<ProjectileManager.Explosion>();
	...
	public void newProjectile(Tower t, Enemy e) {
		...
		if(type == ARROW) {
			float acrValue = (float)Math.atan(yDist / (float)xDist);
			rotate = (float)Math.toDegrees(acrValue);
	...
	private void drawExposions(Graphics2D g2d) {
		for(Explosion e : explosions) {
			if(e.getIndex() < 7)
